...
- **Prompt Engineering**: выбран с этой целью `'This movie is great. {to_generate}'`, он даёт результаты, как показано в соответствующем ноутбуке. Также была попытка вставить другие ключевые слова через bias, чтобы побуждать модель генерировать нужное.
